    ...
    for i in 0..m {
        for j in 0..n {
            if seq1[i] == seq2[j] {
                ...
                let v1 = c.get(i + 1, j).unwrap();
                let v2 = c.get(i, j + 1).unwrap();
                if v1 < v2 {
                    c.set(i + 1, j + 1, v2).unwrap();
                } else {
                    c.set(i + 1, j + 1, v1).unwrap();
                }
            }
        }
    }
...
fn main() {
    ...
    let lines1 = read_file_lines(filename1).expect("Please provide valid file");
    let lines2 = read_file_lines(filename2).expect("Please provide valid file");
